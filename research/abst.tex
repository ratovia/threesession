...
CoMayaではオブジェクト単位の同期となる. より柔軟に同期編集するためには, 頂点の操作も含めた3次元データの同期をする必要があるが,
3次元データはテキストデータに比べ構造が複雑な為, クライアント間で編集の衝突が起こる可能性がある.
一方, 衝突対策の研究としてWebページの同期編集を可能とするシステムWFE-Sがある\cite{WFE-S}.
