...
同期編集に関する研究は多数行われており, その一つとしてWebページの同期編集を可能とするシステムWFE-Sの研究がある\cite{WFE}.
WFE-Sでは, 各クライアント間の一貫性を保持するために, 編集前と編集後の差分データによる差分同期の方法が提案されている.
また, 3Dモデリングの同期編集を可能とするCoMayaの研究がある\cite{CoMata}.
しかし, CoMayaではオブジェクト単位の同期となり, オブジェクトを構成する頂点の操作においての同期は実装されていない.
本研究では, オブジェクトの頂点の操作を含めた3Dモデリングにおける3次元データを対象とする.
従来のテキストデータより構造が複雑な3次元データに, 差分データによる差分同期の方法を適用する場合,
データ管理において3次元データならではの同期編集システムが必要である.
...
\section{システム}
本システムは, ユーザによる編集が可能なクライアントと, 頂点編集のデータを管理するサーバプログラムによって構成されるモデリングソフトである.
...
衝突回避処理は, 頂点編集の範囲が他のクライアントと重なっている場合, 一方を採用し, 採用されなかった頂点編集のデータを無効にする.
衝突した操作の組み合わせパターンに応じて採用する方を決定する.
...
今後は, 衝突回避処理の詳細を実装する. さらに, モデリングに必要な操作の拡張を行うことでシステムの完成を目指す.
