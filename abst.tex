...
本研究では, 3Dモデリングの3次元データを対象とする.
従来のテキストデータより構造が複雑な3次元データを扱う場合, 衝突回避処理やデータ管理において3Dモデリングならではの同期編集システムが必要である.

\section{目的}
本研究では, 3Dモデリングならではの編集内容の衝突やデータ管理を考慮した同期編集システムの手法を提案する.
これにより, 3Dモデリングソフトにおいて複数人で頂点データを共有し同期編集可能となる.

\section{システム}
本システムは, ユーザによる頂点編集が可能なクライアントと, 頂点編集のデータを管理するサーバプログラムによって構成されるモデリングアプリケーションである.
また, 複数クライアントでの編集結果の衝突回避に頂点編集の差分データを用いる.
クライアント間では, サーバを介して, (1)データの送信, (2)衝突回避処理, (3)差分データの管理, (4)データの受信, という4つの手順によりデータをリアルタイムに同期する.
