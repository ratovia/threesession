...
この同期誤差を小さくするためにWFE-Sによる差分同期の方法が提案されている\cite{WFE-S}.\par
...
衝突回避処理は, 頂点編集の範囲が他のクライアントと重なっている場合, サーバが最後に受け取ったデータを採用し, 採用されなかった頂点編集のデータを無効にする.
...
\section{進捗と今後の方針}
現在, WebGLのライブラリであるThree.jsを用いて, 頂点編集のデータを送受信可能なクライアントを開発した.
また, Ruby on RailsとMySQLを用いて差分データを管理する簡易なサーバを実装した.
今後は, ユーザ情報を付与しクライアントを識別できるようにする. さらに, サーバの衝突回避処理を実装することでシステムの完成を目指す.
