...
また, クライアントは一定時間ごとにデータをサーバから受信する. これらの手順によりデータの同期をする.
...
\subsection{衝突回避処理}
クライアントが, 同期誤差が発生している状態で頂点編集のデータを送信する場合に, 編集内容の衝突が起こる可能性がある.
...
\subsection{データの受信}
クライアントは, 一定時間ごとにサーバへポーリングすることでデータの受信要求をすると, サーバは最新のスナップショットとそれ以降の差分データをクライアントに送信する.
...
今後は, Ruby on Railsでクライアントとサーバのデータ通信の部分を実装していく.
